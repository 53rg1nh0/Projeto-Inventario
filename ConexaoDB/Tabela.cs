...
namespace ConexaoDB
{
    public class Tabela
    {
        ...
        public static void AtualizarUnidades()
        {
            ...
            Unidades = conexao.SqlCapturar("SELECT SIGLA,UNIDADE,UF,REGIAO FROM UNIDADES ORDER BY UF");
        }

        public static void AtualizarClientes()
        {
            Conexao conexao = new Conexao();
            Clientes = conexao.SqlCapturar("SELECT MATRICULA, NOME, USERID, AREA, CARGO, SIGLA_UNIDADE, PATRIMONIO FROM CLIENTE ORDER BY NOME");
        }
        public static void AtualizarResponsaveis()
        {
            Conexao conexao = new Conexao();
            var  temp = conexao.SqlCapturar("SELECT C.NOME AS NOME, C.USERID AS USERID, C.AREA AS AREA, C.CARGO AS CARGO, R.TELEFONECORPORATIVO AS TELEFONECORPORATIVO, R.TELEFONEPESSOAL AS TELEFONEPESSOAL, R.EMAIL AS EMAIL FROM RESPONSAVEL R LEFT JOIN CLIENTE C ON R.USERID_CLIENTE = C.USERID ORDER BY C.NOME");
